...
if "__main__" == __name__:
    ...
    # Filter to only downloaded scenes
    downloaded_scenes = [d for d in os.listdir(dataset_dir) if d.startswith('ai_') and os.path.isdir(os.path.join(dataset_dir, d))]
    print(f"Found {len(downloaded_scenes)} downloaded scenes: {sorted(downloaded_scenes)}")
    meta_df = meta_df[meta_df.scene_name.isin(downloaded_scenes)].copy()
    print(f"Filtered CSV to {len(meta_df)} rows matching downloaded scenes")
