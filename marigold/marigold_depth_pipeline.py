...
class MarigoldDepthPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        text_encoder: CLIPTextModel,
        tokenizer: CLIPTokenizer,
        aggregator: Optional[SlotAggregator] = None,
        scale_invariant: Optional[bool] = True,
        shift_invariant: Optional[bool] = True,
        default_denoising_steps: Optional[int] = None,
        default_processing_resolution: Optional[int] = None,
        num_slots: Optional[int] = 16,
        ...
        self.vae_mid_features = None
        self._register_vae_hook()
        # 注册配置
    ...
    def _register_vae_hook(self):
        """Register forward hook to extract VAE encoder intermediate features"""
        def hook_fn(module, input, output):
            self.vae_mid_features = output

        # Hook at down_blocks[2] output: (B, 512, 32, 32)
        self.vae.encoder.down_blocks[2].register_forward_hook(hook_fn)
    ...
    def single_infer(
        ...
        if num_slots < 77:
            ...
            slots = torch.cat([slots, padding], dim=1)  # [B, 77, slot_dim]
        ...
        # Use slots instead of empty text embedding
        # batch_empty_text_embed is replaced by slots from aggregator
        ...
        for i, t in iterable:
            ...
            noise_pred = self.unet(
                unet_input, t, encoder_hidden_states=slots
            ).sample  # [B, 4, h, w]
    ...
    def encode_rgb(self, rgb_in: torch.Tensor) -> torch.Tensor:
        """
        Encode RGB image into latent and extract intermediate features.
        ...
        Returns:
            `torch.Tensor`: Image latent (B, 4, 32, 32).

        Note:
            VAE intermediate features are stored in self.vae_mid_features (B, 512, 32, 32)
            via the registered hook.
        """
        # encode
